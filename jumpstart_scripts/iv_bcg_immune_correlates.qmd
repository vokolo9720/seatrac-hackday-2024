...
# data_dir = '/Volumes/fh/fast/gilbert_p/fg_data/SEATRAC/TB_hackday_2024/data'
data_dir = '/fh/fast/gilbert_p/fg_data/SEATRAC/TB_hackday_2024/processed_data'

#darrah  = file.path(data_dir, "darrah_etal")
#liu     = file.path(data_dir, "liu_etal")
```
...
```{r}
#| warning: false
#gex_tbl = readr::read_tsv(file.path(liu,"GSE218270_ivbcg_dosecohort_processed.txt.gz"))
#rmeta_tbl = readr::read_csv(file.path(liu,"GSE218270_family.soft.csv"))
#rmeta_tbl <- column_to_rownames(as.data.frame(rmeta_tbl), var = "description")
#correlates_tbl = readr::read_csv(file.path(liu,"GSE218270_family.soft.csv.immune_correlates.csv"))
...
As a first task, consider Figure 1D from Liu et al. 2023. It show the number of cytokine producing CD4 and CD8 T cells sampled from the lungs via BAL (y-axis) versus i.v. BCG dose (x-axis). The median response is shown by the bold colored lines.
...
IMPORTANT NOTE: the `<key>` column includes timepoint or "nAUC", whereas the short_key column does not. If you filter by `<short_key>` column you will return data from all time points, which is what we want for figure 1!!!
...
response_by_dose_plot <- function(var1 = "(L)#CD4:AnyG2T17") {
  ...
  # We compute the median at each timepoint for each binned group
...
Notice that a (+) or (-) indicates the presence and absences of cytokines in that number of cells with that speciifc combination. There are only 10 combination IFNg + CD4+ boolean combinations. Later on you may wish to examine two interesting ones — `(L)%CD4:G+2+17-T-")` vs. `(L)#CD4:G+2-17-T+` . these are similar in their degree of poly functionality but differ in co-expression: `IFNg+IL2+IL17-TNF-` vs `IFNg+IL2-IL17-TNF+` .[Darrah et al. 2023](https://pubmed.ncbi.nlm.nih.gov/37267955/) emphasized that the frequency of CD4 T cells producing TNF with interferon γ (IFNγ) was one of key signatures of protection.

### Try this: reuse the code with different variables

With the same code you can explore any of the 100s of variables in the data set. HExplore more the variable in `correlates_tbl` columns and use `<short_key>` to interrogate more variables. For instance,
...
Run the code. What do you notices about the intermediate dose group? Do those withj more robust T cell responses to BCG (i.e., more cytokine producing cells) tend to be protected from subsequent challenge. (In case you are curious – the T cell response is measured *ex vivo* via an intercellular cytokine staining assays where PBMCs were stimulated using a tuberculin protein [PPD skin test antigen](https://www.ncbi.nlm.nih.gov/books/NBK556037/)).
...
Change the code to examine other variables. Try "`(L)%CD4:G+2+17-T-"` and `"(L)%CD4:G+2-17-T+"` . What do you notice.
...
```{r}
#| warning: false
# Function returns p-value from Wilcoxon unpaired test - the liklihood that that rank difference between groups might arise by chance.
...
Here we tether tegether the `group_by` , `group_split` and `purrr::map` to apply `test_function()` and `diff_function()` across each group split.
...
x = rmeta_tbl %>% 
  ...
  tally() %>%
  select(animalid, protect_outcome) %>% 
...
key_extra = correlates_tbl %>% group_by(key) %>% slice(1) %>%
  select(key, short_key, tissue, unit, tp)
...
Above we note that baseline blood cholesterol was perhaps higher in non-protected monkeys. Look at the cholestrol distribution=

```{r}
#| warning: false
#| fig-width: 4
#| fig-height: 4
var_name =  "CHOLESTEROL - mg/dL 0"
x %>% filter(key == var_name ) %>% 
...
## Try this: Examine cholesterol and protection status longitudinally

Let's see how cholesterol levels are distributed across the dose groups. What do you notice

```{r}
#| warning: false
#| fig-width: 6
#| fig-height: 3
gg1 =  response_by_dose_protection_plot(var1 = "(P)Chol")
gg1
```

## Try this: Examine a baseline variable associated with protection

```{r}
#| warning: false
#| fig-width: 4
#| fig-height: 4
var_name = "PBMC/Any/CD8/Percent Granzyme B 0"
x %>% filter(key == var_name) %>% 
  ggplot(aes(x=protect_outcome, y = value)) + 
  geom_boxplot() + 
  geom_point() + 
...
#| fig-width: 6
#| fig-height: 3
gg1 =  response_by_dose_protection_plot(var1 = "(P)%CD8(Any+):GrzB")
gg1
```
...
Suppose you want to evaluate gene expression with a specific immune variable. Suppose you select `Bool/CD4/FoP/g+2-17-T+ 8` This is from the BAL and Boolean combination of CD4 T cells that are IFNg+ IL2- IL17- and TNF+ at timepoint 8.
...
# join hte metadata with the immunlogical variable by animalid
...
diffs_tbl %>% 
  ...
  arrange(mean_ratio) %>% 
  select(short_key, variable_name, mean_ratio, wilcox_p, tissue2, tp ) %>% 
  filter(short_key == "(L)%CD4:G+2+17-T-")
```
...
#| fig-height: 8
gg2 =  response_by_dose_protection_plot(var1 = "(L)%CD4:G+2+17-T-")
gg3 =  response_by_dose_protection_plot(var1 = "(L)#CD4:G+2-17-T+")
gridExtra::grid.arrange(gg2,gg3, ncol =1)
```
